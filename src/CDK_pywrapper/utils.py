...
def make_temp_jre() -> Tuple[str, str]:
    ...
    install_path = [
        path for path in Path(install_jre()).parents
        if path.as_posix().endswith("jre") and not path.as_posix().endswith(".jre")
    ][0]
    # Path of temp dir
    outdir = mktempdir()
    # Make copy into dir
...
def get_java_in_dir(dir: str, version: int):
    """Recursively search the directory to find a JRE."""
    paths = glob.glob(os.path.join(dir, "**", "bin", "java.exe" if sys.platform == "win32" else "java"), recursive=True)
...
def mktempdir(suffix: Optional[str] = None) -> str:
    """Return the path to a writeable temporary directory."""
    dir = tempfile.mkdtemp(suffix=suffix)
    return dir


def mktempfile(suffix: Optional[str] = None) -> str:
    ...
    for atom in mol.GetAtoms():
        ...
        for nbr in atom.GetNeighbors():
            if nbr.GetAtomicNum() == 1:
                n_h_nbrs += 1
        no_neighbors = False
        if atom.GetTotalNumHs(no_neighbors) > n_h_nbrs:
            return True
    return False
